...
class Mavlers_CF_Integrations {
    ...
    private function process_mailchimp_integration($submission_id, $form_data, $settings) {
        error_log('🔧 Old Mailchimp Integration: Starting process for submission ' . $submission_id);
        ...
        if (empty($api_key) || empty($list_id) || empty($email_field)) {
            error_log('❌ Old Mailchimp Integration: Missing required settings');
            return;
        }
        ...
        if (empty($email)) {
            error_log('❌ Old Mailchimp Integration: No email value found');
            return;
        }
        
        error_log('📧 Old Mailchimp Integration: Email=' . $email . ', Name=' . $name);
        ...
        error_log('🚀 Old Mailchimp Integration: Making API request to ' . $url);
        $response = wp_remote_request($url, $args);
        
        if (is_wp_error($response)) {
            error_log('❌ Old Mailchimp Integration: Request error - ' . $response->get_error_message());
            return;
        }
        ...
        error_log('📋 Old Mailchimp Integration: Response Code=' . $response_code);
        error_log('📋 Old Mailchimp Integration: Response Body=' . $response_body);
        
        if ($response_code === 200) {
            error_log('✅ Old Mailchimp Integration: Successfully added subscriber');
        } else {
            error_log('❌ Old Mailchimp Integration: Failed with code ' . $response_code);
        }
    }
    ...
    private function process_hubspot_integration($submission_id, $form_data, $settings) {
        error_log('🔧 Old HubSpot Integration: Starting process for submission ' . $submission_id);
        ...
        if (empty($api_key) || empty($email_field)) {
            error_log('❌ Old HubSpot Integration: Missing required settings');
            return;
        }
        ...
        if (empty($email)) {
            error_log('❌ Old HubSpot Integration: No email value found');
            return;
        }
        ...
        error_log('🚀 Old HubSpot Integration: Making API request');
        $response = wp_remote_request($url, $args);
        
        if (is_wp_error($response)) {
            error_log('❌ Old HubSpot Integration: Request error - ' . $response->get_error_message());
            return;
        }
        ...
        error_log('📋 Old HubSpot Integration: Response Code=' . $response_code);
        
        if ($response_code === 201) {
            error_log('✅ Old HubSpot Integration: Successfully created contact');
        } else {
            error_log('❌ Old HubSpot Integration: Failed with code ' . $response_code);
        }
    }
} 