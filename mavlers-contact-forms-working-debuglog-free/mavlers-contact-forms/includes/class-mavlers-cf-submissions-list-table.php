...
class Mavlers_CF_Submissions_List_Table extends WP_List_Table {
    ...
    public function __construct($form_id = 0) {
        ...
        error_log('Mavlers CF: Submissions list table - Form ID: ' . $form_id);
        ...
        if ($form_id) {
            ...
            if ($form) {
                ...
                error_log('Mavlers CF: Submissions list table - Form fields loaded: ' . print_r($this->form_fields, true));
            } else {
                error_log('Mavlers CF: Submissions list table - Form not found for ID: ' . $form_id);
            }
        } else {
            error_log('Mavlers CF: Submissions list table - No form ID provided');
        }
    }
