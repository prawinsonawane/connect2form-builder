		...
		if ( class_exists( '\Connect2Form\Integrations\Core\Services\ServiceManager' ) ) {
			...
			if ( function_exists( 'c2f_is_valid_prefixed_table' ) && ! c2f_is_valid_prefixed_table( $table ) ) {
				return;
					...
					// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- cleanup operation for old submissions with validated table identifier; no caching needed for cleanup operations
			$wpdb->query( $wpdb->prepare(
				// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- table name is validated with c2f_is_valid_prefixed_table and built from trusted prefix
