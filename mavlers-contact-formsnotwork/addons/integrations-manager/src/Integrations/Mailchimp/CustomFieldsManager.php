...
class CustomFieldsManager {
    ...
    public function get_merge_fields($audience_id, $force_refresh = false) {
        ...
        if (!$this->mailchimp_integration->is_connected()) {
            return array('success' => false, 'message' => 'Mailchimp integration not connected');
        }
        ...
        $endpoint = "/lists/{$audience_id}/merge-fields?count=100";
        $response = $this->make_mailchimp_request('GET', $endpoint, $api_key, $dc);
        
        if (!$response) {
            return array('success' => false, 'message' => 'Failed to fetch merge fields from Mailchimp');
        }
        
        if ($response['status'] !== 200) {
            $error_message = isset($response['body']['detail']) ? $response['body']['detail'] : 'Unknown error';
            return array('success' => false, 'message' => "API Error: {$error_message}");
        }
        
        if (!isset($response['body']['merge_fields'])) {
            return array('success' => false, 'message' => 'Invalid response from Mailchimp API');
        }
        
        $merge_fields = $response['body']['merge_fields'];
        ...
        $result = array('success' => true, 'data' => $processed_fields);
    ...
    public function create_merge_field($audience_id, $field_data) {
        ...
        if (!$this->mailchimp_integration || !$this->mailchimp_integration->is_connected()) {
    ...
    private function make_mailchimp_request($method, $endpoint, $api_key, $dc, $data = array()) {
        ...
        return array('status' => $response_code, 'body' => $decoded);
    }
