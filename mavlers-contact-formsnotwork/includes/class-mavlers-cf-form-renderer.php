...
class Mavlers_CF_Form_Renderer {
    ...
    public function render_form($form_id) {
        ...
        // Add basic CSS for form messages
        $form_html .= '<style>
        .mavlers-cf-message {
            padding: 10px;
            margin: 10px 0;
        ...
        .mavlers-cf-submit:disabled {
            opacity: 0.6;
            cursor: not-allowed;
        }
        </style>';

        // Add inline JavaScript as fallback to ensure form submission works
        $nonce = wp_create_nonce('mavlers_cf_submit');
        ...
        (function($) {
            ...
            $(document).ready(function() {
                ...
                console.log("Mavlers CF Debug: jQuery loaded:", typeof $ !== "undefined");
                console.log("Mavlers CF Debug: Form found:", $(".mavlers-cf-form").length);
                
                // Ensure we have the form element
                var $form = $(".mavlers-cf-form");
                if ($form.length === 0) {
                    console.error("Mavlers CF: No form found with class .mavlers-cf-form");
                    return;
                }

                // Remove any existing submit handlers to avoid duplicates
                $form.off("submit.mavlers-cf");
                
                // Add our submit handler
                $form.on("submit.mavlers-cf", function(e) {
                    e.preventDefault();
                    console.log("Mavlers CF: Form submit intercepted");
                    
                    var $currentForm = $(this);
                    var $submit = $currentForm.find(".mavlers-cf-submit");
                    var $message = $currentForm.find(".mavlers-cf-message");
                    ...
                    if (!isValid) {
                        $submit.prop("disabled", false).text("' . esc_js($submit_text) . '");
                        $message.addClass("error").text("Please fill in all required fields.").show();
                        return;
                    }
                    ...
                    console.log("Mavlers CF: Submitting to:", "' . esc_js($ajax_url) . '");
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            console.log("Mavlers CF: Response received:", response);
                            if (response.success) {
                                $message.addClass("success").text(response.data.message || "Form submitted successfully!").show();
                                $currentForm[0].reset();
                                ...
                                var errorMsg = response.data || "An error occurred. Please try again.";
                                $message.addClass("error").text(errorMsg).show();
                                console.error("Mavlers CF: Submission error:", errorMsg);
                            }
                        },
                        error: function(xhr, status, error) {
                            console.error("Mavlers CF: AJAX error:", {xhr: xhr, status: status, error: error});
                ...
                console.log("Mavlers CF: Form handler attached successfully");
            });
        })(jQuery);
        </script>';
