 ...
 * Tested up to: 6.8
 * Requires PHP: 7.4
 * Author: pravinsonawnae,pravinsonawane71,sonymerlinn
 * Author URI: https://profiles.wordpress.org/pravinsonawane71/
 * Text Domain: connect2form-builder
...
if ( version_compare( get_bloginfo( 'version' ), CONNECT2FORM_MIN_WP_VERSION, '<' ) ) {
    add_action( 'admin_notices', function() {
        // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Admin notice with proper escaping
             ...
             esc_html( sprintf( /* translators: %s: Minimum WordPress version */ __( 'Connect2Form requires WordPress %s or higher. Please upgrade WordPress.', 'connect2form-builder'), CONNECT2FORM_MIN_WP_VERSION ) ) .
             '</p></div>';
    } );
    return;
}
...
if ( version_compare( PHP_VERSION, CONNECT2FORM_MIN_PHP_VERSION, '<' ) ) {
    add_action( 'admin_notices', function() {
        // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Admin notice with proper escaping
        echo '<div class="notice notice-error"><p>' .
            esc_html( sprintf( /* translators: %s: Minimum PHP version */ __( 'Connect2Form requires PHP %s or higher. Please contact your hosting provider.', 'connect2form-builder'), CONNECT2FORM_MIN_PHP_VERSION ) ) .
            '</p></div>';
    } );
    return;
}
...
// Integration Manager addon constants - now handled by separate plugin
// These constants are defined by the Connect2Form Integrations plugin if it's active
...
function run_connect2form() {
    ...
    if ( is_admin() ) {
        $admin = new Connect2Form_Admin( 'connect2form-builder', CONNECT2FORM_VERSION );
    ...
    // Load integrations manager addon if available (now handled by separate plugin)
    // The Connect2Form Integrations plugin will hook into WordPress automatically
}
