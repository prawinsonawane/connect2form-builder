jQuery(document).ready(function($) {
    ...
    $('.connect2form-form').on('submit', function(e) {
      ...
      // Add current page URL as referer
      formData.append('wp_http_referer', window.location.href);
     ...
     */
    function showLoadingState($submit, isLoading) {
        if (isLoading) {
            // Store original text
            ...
            $submit.addClass('connect2form-loading')
                   .prop('disabled', true)
                   .html('<span class="connect2form-spinner"></span><span class="connect2form-loading-text">Submitting...</span>');
        } else {
            // Restore original state
            var originalText = $submit.data('original-text') || 'Submit';
            $submit.removeClass('connect2form-loading')
                   .prop('disabled', false)
                   .text(originalText);
        }
    }
