...
if ($form_id) {
    ...
    if (current_user_can('manage_options')) {
        error_log('DEBUG: HubSpot form settings loading - Form ID: ' . $form_id);
        error_log('DEBUG: Meta table value: ' . ($meta_value ?? 'null'));
        error_log('DEBUG: Post meta value: ' . print_r($post_meta ?? [], true));
        error_log('DEBUG: Final form settings: ' . print_r($form_settings, true));
    }
}
...
// Debug logging
if (current_user_can('manage_options')) {
    error_log('DEBUG: HubSpot form settings template - Form ID: ' . $form_id);
    error_log('DEBUG: HubSpot form settings template - Form settings: ' . print_r($form_settings, true));
    error_log('DEBUG: HubSpot form settings template - Field mapping: ' . print_r($form_settings['field_mapping'] ?? [], true));
}
?>
