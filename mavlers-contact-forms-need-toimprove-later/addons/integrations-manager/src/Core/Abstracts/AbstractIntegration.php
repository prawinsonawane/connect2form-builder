...
abstract class AbstractIntegration implements IntegrationInterface {
    ...
    protected function getFormSettings(int $form_id): array {
        if (current_user_can('manage_options')) {
            error_log('DEBUG: AbstractIntegration::getFormSettings - Form ID: ' . $form_id . ', Integration: ' . $this->getId());
        }
        
        $form_settings = get_post_meta($form_id, '_mavlers_cf_integrations', true);
        
        if (current_user_can('manage_options')) {
            error_log('DEBUG: Retrieved form settings: ' . print_r($form_settings, true));
        }
        
        $integration_settings = $form_settings[$this->getId()] ?? [];
        
        if (current_user_can('manage_options')) {
            error_log('DEBUG: Integration settings for ' . $this->getId() . ': ' . print_r($integration_settings, true));
        }
    ...
    protected function saveFormSettings(int $form_id, array $settings): bool {
        if (current_user_can('manage_options')) {
            error_log('DEBUG: AbstractIntegration::saveFormSettings - Form ID: ' . $form_id . ', Integration: ' . $this->getId());
        ...
        if (current_user_can('manage_options')) {
            error_log('DEBUG: Existing form settings: ' . print_r($form_settings, true));
        }
        
        $form_settings[$this->getId()] = $settings;
        
        if (current_user_can('manage_options')) {
            error_log('DEBUG: Updated form settings: ' . print_r($form_settings, true));
        }
        ...
        if (current_user_can('manage_options')) {
            error_log('DEBUG: Save result: ' . ($result ? 'true' : 'false'));
            error_log('DEBUG: Verification settings: ' . print_r($verify_settings, true));
            error_log('DEBUG: Integration settings: ' . print_r($integration_settings, true));
            error_log('DEBUG: Verification success: ' . ($verification_success ? 'true' : 'false'));
        }
