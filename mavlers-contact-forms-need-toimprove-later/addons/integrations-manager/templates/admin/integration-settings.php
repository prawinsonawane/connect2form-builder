...
if ($integration_id === 'hubspot') {
    ...
    wp_enqueue_script(
        'mavlers-cf-hubspot',
        MAVLERS_CF_INTEGRATIONS_URL . 'assets/js/admin/hubspot.js',
        ['jquery', 'wp-util'],
        '2.0.0',
        true
    );
...
jQuery(document).ready(function($) {
    ...
    // Top-right corner message function
    function showTopRightMessage(message, type) {
        // Create or update message element
        let messageElement = $('#integration-message');
        if (messageElement.length === 0) {
            messageElement = $('<div id="integration-message" style="position: fixed; top: 20px; right: 20px; z-index: 9999; padding: 15px; border-radius: 4px; color: white; font-weight: 500;"></div>');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    showTopRightMessage(successMessage, 'success');
                    ...
                    showTopRightMessage(errorMessage, 'error');
                }
            },
            error: function(xhr, status, error) {
