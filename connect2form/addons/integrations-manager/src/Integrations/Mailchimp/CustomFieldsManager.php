...
/**
 * Mailchimp Custom Fields Manager Class
 *
 * @since    1.0.0
 */
class CustomFieldsManager {
	 ...
	 * @var      string
	 */
	private $cache_group = 'connect2form_mailchimp_fields';
	 ...
	 *
	 * @since    1.0.0
	 * @param    object $mailchimp_integration Mailchimp integration instance.
	 */
	...
	private function init_hooks() {
		...
		// Field validation.
		add_filter( 'connect2form_mailchimp_validate_field_data', array( $this, 'validate_custom_field_data' ), 10, 3 );
	...
	public function enqueue_custom_fields_assets( $hook ) {
		if ( strpos( $hook, 'connect2form' ) !== false || strpos( $hook, 'connect2form' ) !== false ) {
			wp_enqueue_style(
				'connect2form-custom-fields',
				CONNECT2FORM_INTEGRATIONS_URL . 'assets/css/admin/custom-fields.css',
				array(),
				'1.0.0'
			);
	 ...
	 *
	 * @since    1.0.0
	 * @param    array $merge_fields Merge fields from API.
	 * @return   array
	 */
	...
	private function get_field_validation_rules( $type, $options = array() ) {
		...
		switch ( $type ) {
			case 'email':
				...
				$rules['message'] = __( 'Please enter a valid email address', 'connect2form' );
				break;
				
			case 'number':
				$rules['pattern'] = '^-?\d*\.?\d+$';
				$rules['message'] = __( 'Please enter a valid number', 'connect2form' );
				break;
				
			case 'phone':
				$rules['pattern'] = '^[\+]?[1-9][\d]{0,15}$';
				$rules['message'] = __( 'Please enter a valid phone number', 'connect2form' );
				break;
				
			case 'date':
				$rules['pattern'] = '^\d{4}-\d{2}-\d{2}$';
				$rules['format'] = 'YYYY-MM-DD';
				$rules['message'] = __( 'Please enter a valid date (YYYY-MM-DD)', 'connect2form' );
				break;
				
			case 'birthday':
				$rules['pattern'] = '^(0[1-9]|1[0-2])/(0[1-9]|[12]\d|3[01])$';
				$rules['format'] = 'MM/DD';
				$rules['message'] = __( 'Please enter a valid birthday (MM/DD)', 'connect2form' );
				break;
				
			case 'address':
				$rules['required_subfields'] = array( 'addr1', 'city', 'state', 'zip' );
				$rules['message'] = __( 'Please enter a complete address', 'connect2form' );
				break;
				
			case 'dropdown':
				if ( ! empty( $options['choices'] ) ) {
					$rules['allowed_values'] = wp_list_pluck( $options['choices'], 'label' );
					$rules['message'] = __( 'Please select a valid option', 'connect2form' );
				}
				break;
			...
			default:
				if ( ! empty( $options['size'] ) ) {
					...
					$rules['message'] = sprintf( __( 'Text must be %d characters or less', 'connect2form' ), $rules['max_length'] );
				}
				break;
		}
	 ...
	 * @since    1.0.0
	 * @param    string $audience_id Audience ID.
	 * @param    string $merge_id    Merge field ID.
	 * @param    array  $field_data  Field data.
	 * @return   array
	 */
	 ...
	 * @param    array  $formatting_rules Formatting rules.
	 * @return   mixed
	 */
	 ...
	 *
	 * @since    1.0.0
	 * @param    bool  $is_valid     Whether the field is currently valid.
	...
	public function ajax_get_merge_fields() {
		...
		$force_refresh = filter_var( wp_unslash( $_POST['force_refresh'] ?? '' ), FILTER_VALIDATE_BOOLEAN ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
	...
	public function ajax_create_merge_field() {
		...
		$audience_id = sanitize_text_field( wp_unslash( $_POST['audience_id'] ?? '' ) );
		$field_data = wp_unslash( $_POST['field_data'] ?? array() ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- sanitized below via sanitize_field_data
	...
	public function ajax_update_merge_field() {
		...
		$audience_id = sanitize_text_field( wp_unslash( $_POST['audience_id'] ?? '' ) );
		$merge_id = sanitize_text_field( wp_unslash( $_POST['merge_id'] ?? '' ) );
		$field_data = wp_unslash( $_POST['field_data'] ?? array() ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- sanitized below via sanitize_field_data
	...
	public function ajax_map_interests() {
		...
		$form_id = intval( wp_unslash( $_POST['form_id'] ?? 0 ) );
		$audience_id = sanitize_text_field( wp_unslash( $_POST['audience_id'] ?? '' ) );
		$mapping = (array) wp_unslash( $_POST['mapping'] ?? array() ); // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- sanitized below
		...
		$option_key = "connect2form_mailchimp_interest_mapping_{$form_id}_{$audience_id}";
