...
/**
 * Mailchimp Batch Processor Class
 *
 * @since    1.0.0
 */
class BatchProcessor {
	...
	public function __construct($api_client = null, $logger = null) {
		global $wpdb;
		$this->batch_table = $wpdb->prefix . 'connect2form_mailchimp_batch_queue';
	...
	private function init_hooks() {
		...
		add_action( 'connect2form_mailchimp_process_batch_queue', array( $this, 'process_batch_queue' ) );
		add_action( 'connect2form_mailchimp_cleanup_batch_queue', array( $this, 'cleanup_old_batches' ) );
		
		// Schedule cron jobs if not already scheduled.
		if ( ! wp_next_scheduled( 'connect2form_mailchimp_process_batch_queue' ) ) {
			wp_schedule_event( time(), 'connect2form_every_5_minutes', 'connect2form_mailchimp_process_batch_queue' );
		}
		
		if ( ! wp_next_scheduled( 'connect2form_mailchimp_cleanup_batch_queue' ) ) {
			wp_schedule_event( time(), 'daily', 'connect2form_mailchimp_cleanup_batch_queue' );
		}
		...
		add_action( 'connect2form_mailchimp_add_to_batch', array( $this, 'add_to_batch_queue' ), 10, 3 );
	...
	private function maybe_create_batch_table() {
		...
		$table = $this->batch_table;
		// phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- validated prefixed table name used in DDL
	...
	public function add_cron_intervals($schedules) {
		$schedules['connect2form_every_5_minutes'] = array(
			'interval' => 300, // 5 minutes.
			'display' => 'Every 5 Minutes'
		);
		
		$schedules['connect2form_every_15_minutes'] = array(
	...
	public function add_to_batch_queue($form_data, $form_settings, $form_id) {
		...
		if ( class_exists( '\Connect2Form\Integrations\Core\Services\ServiceManager' ) ) {
			...
			$queue_item_id = $service_manager->database()->insertBatchQueueItem( $queue_data );
		} else {
			// Fallback to direct database call if service not available.
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- batch queue insert; service layer preferred but this is a fallback
			$result = $wpdb->insert( $this->batch_table, $queue_data );
			$queue_item_id = $result ? $wpdb->insert_id : 0;
		}
	...
	private function get_pending_batches() {
		if ( class_exists( '\Connect2Form\Integrations\Core\Services\ServiceManager' ) ) {
			$service_manager = \Connect2Form\Integrations\Core\Services\ServiceManager::getInstance();
			$items = $service_manager->database()->getPendingBatchItems( $this->max_batch_size );
		} else {
			// Fallback to direct database call if service not available.
			global $wpdb;
			...
			$limit = absint( $this->max_batch_size );
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- batch processing query with validated table identifier; no caching needed for batch operations
			$items = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT * FROM `{$table}` WHERE status IN (%s, %s) ORDER BY priority DESC, created_at ASC LIMIT %d",
					self::STATUS_PENDING,
	 ...
	 * @since    1.0.0
	 * @param    string $audience_id Audience ID.
	 * @param    array  $operations  Mailchimp operations.
	 * @param    array  $items       Batch items.
	 */
	...
	private function make_batch_api_request($batch_data) {
		if ( ! $this->api_client ) {
			throw new \Exception( esc_html__( 'API client not available.', 'connect2form' ) );
		}
		
		return $this->api_client->make_request( 'POST', '/batches', $batch_data );
	}
	 ...
	 *
	 * @since    1.0.0
	 * @param    string $mailchimp_batch_id Mailchimp batch ID.
	 * @param    array  $items              Batch items.
	 */
	private function schedule_batch_completion_check($mailchimp_batch_id, $items) {
		...
		wp_schedule_single_event(
			...
			'connect2form_mailchimp_check_batch_completion',
	...
	public function check_batch_completion($mailchimp_batch_id, $item_ids) {
		...
		$response = $this->api_client->make_request( 'GET', "/batches/{$mailchimp_batch_id}" );
	 ...
	 * @param    string $mailchimp_batch_id Mailchimp batch ID.
	 * @param    array  $item_ids           Item IDs.
	 * @param    array  $response           Mailchimp response.
	 */
	...
	private function get_batch_results($mailchimp_batch_id) {
		...
		$batch_info = $this->api_client->make_request( 'GET', "/batches/{$mailchimp_batch_id}" );
	...
	private function handle_item_error($item_id, $error_message) {
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- batch item retry count query with validated table identifier; no caching needed for batch processing
		$item = $wpdb->get_row( $wpdb->prepare( "SELECT retry_count FROM `{$table}` WHERE id = %d", $item_id ) );
		...
		if ( $retry_count <= $this->max_retries ) {
			// Retry the item.
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- batch retry operation; no caching needed for batch processing
			$wpdb->update(
				$this->batch_table,
				array(
	...
	private function update_batch_status($items, $status) {
		...
		if ( class_exists( '\Connect2Form\Integrations\Core\Services\ServiceManager' ) ) {
			$service_manager = \Connect2Form\Integrations\Core\Services\ServiceManager::getInstance();
			$service_manager->database()->updateBatchItemsStatus( $item_ids, $status );
		} else {
			// Fallback to direct database call if service not available.
			global $wpdb;
			
            $placeholders = implode( ',', array_fill( 0, count( $item_ids ), '%d' ) );
            $args = array_merge( array( $status, current_time( 'mysql' ) ), array_map( 'absint', $item_ids ) );
            $table = $this->batch_table;
            if ( function_exists( 'c2f_is_valid_prefixed_table' ) && ! \c2f_is_valid_prefixed_table( $table ) ) {
                return;
            }
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.ReplacementsWrongNumber -- batch status update with validated table identifier; dynamic IN list bound via $args; no caching needed for batch operations
			$wpdb->query( $wpdb->prepare( "UPDATE `{$table}` SET status = %s, processed_at = %s WHERE id IN ({$placeholders})", $args ) );
		}
	}
	...
	private function update_item_status($item_id, $status, $error_message = null) {
		if ( class_exists( '\Connect2Form\Integrations\Core\Services\ServiceManager' ) ) {
			$service_manager = \Connect2Form\Integrations\Core\Services\ServiceManager::getInstance();
			$service_manager->database()->updateBatchItemStatus( $item_id, $status, $error_message );
		} else {
			// Fallback to direct database call if service not available.
			global $wpdb;
			...
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- batch status update; no caching needed for batch processing
			$wpdb->update(
				$this->batch_table,
				$update_data,
				array( 'id' => $item_id )
	...
	private function update_item_mailchimp_batch_id($item_id, $mailchimp_batch_id) {
		if ( class_exists( '\Connect2Form\Integrations\Core\Services\ServiceManager' ) ) {
			$service_manager = \Connect2Form\Integrations\Core\Services\ServiceManager::getInstance();
			$service_manager->database()->updateBatchItemMailchimpBatchId( $item_id, $mailchimp_batch_id );
		} else {
			// Fallback to direct database call if service not available.
			global $wpdb;
			
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- batch ID update; service layer preferred but this is a fallback
			$wpdb->update(
				$this->batch_table,
	...
	private function get_operation_id_for_item($item_id) {
		if ( class_exists( '\Connect2Form\Integrations\Core\Services\ServiceManager' ) ) {
			$service_manager = \Connect2Form\Integrations\Core\Services\ServiceManager::getInstance();
			$item = $service_manager->database()->getBatchItem( $item_id );
		} else {
			// Fallback to direct database call if service not available.
			global $wpdb;
			...
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- batch item operation type query with validated table identifier; no caching needed for batch processing
			$item = $wpdb->get_row( $wpdb->prepare( "SELECT operation_type FROM `{$table}` WHERE id = %d", $item_id ) );
		}
	...
	private function is_batch_processing_enabled() {
		return get_option( 'connect2form_mailchimp_batch_processing', true );
	}
	...
	private function can_process_batches() {
		...
		$processing_flag = get_transient( 'connect2form_mailchimp_batch_processing' );
		...
		// Set processing flag.
		set_transient( 'connect2form_mailchimp_batch_processing', true, 300 ); // 5 minutes.
	...
	private function maybe_trigger_immediate_processing() {
		if ( class_exists( '\Connect2Form\Integrations\Core\Services\ServiceManager' ) ) {
			$service_manager = \Connect2Form\Integrations\Core\Services\ServiceManager::getInstance();
			$pending_count = $service_manager->database()->getPendingBatchCount();
		} else {
			// Fallback to direct database call if service not available.
			global $wpdb;
            $table = $this->batch_table;
			...
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- batch count query with validated table identifier; no caching needed for real-time batch statistics
		...
		if ( $pending_count <= 10 ) {
			wp_schedule_single_event( time() + 30, 'connect2form_mailchimp_process_batch_queue' );
		}
	}
	...
	public function cleanup_old_batches() {
		$retention_days = apply_filters( 'connect2form_mailchimp_batch_retention_days', 30 );
		
		if ( class_exists( '\Connect2Form\Integrations\Core\Services\ServiceManager' ) ) {
			$service_manager = \Connect2Form\Integrations\Core\Services\ServiceManager::getInstance();
			$deleted = $service_manager->database()->cleanupOldBatchItems( $retention_days );
		} else {
			// Fallback to direct database call if service not available.
			global $wpdb;
			...
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- batch cleanup operation with validated table identifier; no caching needed for cleanup operations
			$deleted = $wpdb->query(
				$wpdb->prepare(
					"DELETE FROM `{$table}` WHERE status IN (%s, %s) AND created_at < DATE_SUB(NOW(), INTERVAL %d DAY)",
	...
	public function get_batch_statistics() {
		if ( class_exists( '\Connect2Form\Integrations\Core\Services\ServiceManager' ) ) {
			$service_manager = \Connect2Form\Integrations\Core\Services\ServiceManager::getInstance();
			$stats = $service_manager->database()->getBatchStatistics();
		} else {
			// Fallback to direct database call if service not available.
			global $wpdb;
			...
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- batch statistics query with validated table identifier; no caching needed for real-time batch statistics
			$stats = $wpdb->get_row(
				$wpdb->prepare(
					"SELECT 
						COUNT(*) as total,
						...
						SUM(CASE WHEN status = %s THEN 1 ELSE 0 END) as retrying
					 FROM `{$table}`
	...
	public function show_batch_admin_notices() {
		...
		if ( $stats['failed'] > 10 ) {
			...
			echo '<p><strong>Mailchimp Batch Queue:</strong> ' . esc_html( $stats['failed'] ) . ' items have failed processing. ';
		...
		if ( $stats['pending'] > 100 ) {
			...
			echo '<p><strong>Mailchimp Batch Queue:</strong> ' . esc_html( $stats['pending'] ) . ' items are pending processing.</p>';
			echo '</div>';
		}
	}
	...
	private function log_batch_processing_complete() {
		...
		delete_transient( 'connect2form_mailchimp_batch_processing' );
	}
	...
	public function ajax_retry_failed_batches() {
		...
		if ( class_exists( '\Connect2Form\Integrations\Core\Services\ServiceManager' ) ) {
			$service_manager = \Connect2Form\Integrations\Core\Services\ServiceManager::getInstance();
			$updated = $service_manager->database()->retryFailedBatchItems();
		} else {
			// Fallback to direct database call if service not available.
			global $wpdb;
			
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- batch retry operation; service layer preferred but this is a fallback
			$updated = $wpdb->update(
				$this->batch_table,
				array(
					'status' => self::STATUS_PENDING,
	...
	public function ajax_clear_batch_queue() {
		...
		if ( class_exists( '\Connect2Form\Integrations\Core\Services\ServiceManager' ) ) {
			$service_manager = \Connect2Form\Integrations\Core\Services\ServiceManager::getInstance();
			$deleted = $service_manager->database()->clearCompletedFailedBatchItems();
		} else {
			// Fallback to direct database call if service not available.
			global $wpdb;
			...
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- batch clear operation with validated table identifier; no caching needed for cleanup operations
			$deleted = $wpdb->query(
				$wpdb->prepare(
	...
	public function ajax_process_batch_now() {
		...
		wp_schedule_single_event( time(), 'connect2form_mailchimp_process_batch_queue' );
