/**
 * Connect2Form - Integrations Admin JavaScript - Optimized Version
 * 
...
(function($) {
     ...
     */
    window.Connect2FormIntegrationsAdmin = {
        ...
        handleTestConnection: function(e) {
            ...
            if (!integrationId) {
                this.showNotice('error', connect2formIntegrations.strings.error);
                return;
            }
        ...
        testConnection: function(integrationId, $button) {
            ...
            // Show loading state
            $button.html('<span class="spinner"></span> ' + connect2formIntegrations.strings.testing_connection)
                   .prop('disabled', true);
            ...
            $.ajax({
                url: connect2formIntegrations.ajaxUrl,
                type: 'POST',
                data: {
                    action: 'connect2form_test_integration',
                    nonce: connect2formIntegrations.nonce,
                ...
                success: (response) => {
                    if (response.success) {
                        this.showConnectionResult('success', response.data?.message || connect2formIntegrations.strings.connection_successful);
                        $button.removeClass('button-secondary').addClass('button-primary');
                    } else {
                        this.showConnectionResult('error', response.data || connect2formIntegrations.strings.connection_failed);
                    }
                },
                error: () => {
                    this.showConnectionResult('error', connect2formIntegrations.strings.connection_failed);
                },
                complete: () => {
        ...
        handleSettingsSubmit: function(e) {
            ...
            // Show loading state
            $submitBtn.html('<span class="spinner"></span> ' + connect2formIntegrations.strings.saving)
                     .prop('disabled', true);

            const formData = new FormData(e.target);
            formData.append('action', 'connect2form_save_integration_settings');
            formData.append('nonce', connect2formIntegrations.nonce);
            ...
            $.ajax({
                ...
                success: (response) => {
                    if (response.success) {
                        this.showNotice('success', response.data || connect2formIntegrations.strings.saved);
                        this.refreshPageData();
                    } else {
                        this.showNotice('error', response.data || connect2formIntegrations.strings.error);
                    }
                },
                error: () => {
                    this.showNotice('error', connect2formIntegrations.strings.error);
        ...
        handleCardClick: function(e) {
            ...
            if (integrationId) {
                ...
                window.location.href = connect2formIntegrations.pluginUrl + 'admin.php?page=connect2form-integrations&tab=settings&integration=' + integrationId;
            }
        },
        ...
        refreshIntegrationStatus: function(integrationId) {
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'connect2form_get_integration_data',
                    nonce: connect2formIntegrations.nonce,
                    integration_id: integrationId,
                    data_type: 'status'
                },
    ...
    $(document).ready(function() {
        Connect2FormIntegrationsAdmin.init();
    });

    // Production build - debugging functions removed
