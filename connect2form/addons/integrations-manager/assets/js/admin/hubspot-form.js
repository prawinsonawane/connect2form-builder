...
(function($) {
    ...
    function initializeHubSpotSettings() {
        ...
        // Ensure required variables are available
            if (typeof connect2formCFHubspot === 'undefined') {
        console.error('connect2formCFHubspot is not defined');
        return;
    }
    ...
    function initializeEventHandlers() {
        ...
        // HubSpot form selection change
        $('#hubspot_form_select').on('change', handleHubSpotFormChange);
        ...
        $('#hubspot-clean-mappings').on('click', handleCleanMappings);
    ...
    function handleObjectTypeChange() {
        ...
        const hubspotFormRow = $('#hubspot-form-selection-row');
        ...
        // Show/hide relevant sections based on object type
        ...
        } else if (objectType === 'forms') {
            hubspotFormRow.show();
            loadHubSpotForms();
            // Field mapping will be shown when form is selected
        } else {
            if (objectType && objectType !== 'custom' && objectType !== 'forms') {
    ...
    /**
     * Handle HubSpot form selection change
     */
    function handleHubSpotFormChange() {
        const selectedForm = $('#hubspot_form_select').val();
        const fieldMappingSection = $('#hubspot-field-mapping-section');
        
        if (selectedForm) {
            fieldMappingSection.show();
            loadHubSpotFormFields(selectedForm);
        } else {
            fieldMappingSection.hide();
        }
    }
    ...
    function loadCustomObjects() {
        ...
        $.ajax({
            url: connect2formCFHubspot.ajaxUrl,
            type: 'POST',
            data: {
                action: 'connect2form_hubspot_get_custom_objects',
                nonce: connect2formCFHubspot.nonce,
    ...
    function loadObjectProperties(objectType) {
        const actionMap = {
            'contacts': 'connect2form_hubspot_get_contact_properties',
            'deals': 'connect2form_hubspot_get_deal_properties',
            'companies': 'connect2form_hubspot_get_company_properties'
        };
    ...
    function loadCustomObjectProperties(objectName) {
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'connect2form_hubspot_get_custom_object_properties',
                nonce: connect2formCFHubspot.nonce,
                form_id: currentFormId,
    ...
    function loadHubSpotForms() {
        const formSelect = $('#hubspot_form_select');
        formSelect.html('<option value="">Loading HubSpot forms...</option>');
        
        const ajaxData = {
            action: 'connect2form_hubspot_get_forms',
            nonce: connect2formCFHubspot.nonce,
            form_id: currentFormId
        };
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success && response.data && response.data.forms) {
                    updateHubSpotFormOptions(response.data.forms);
                } else {
                    // Show specific error message if available
                    if (response.data && typeof response.data === 'string') {
                        if (response.data.includes('permissions') || response.data.includes('access token')) {
                            formSelect.html('<option value="">⚠️ Permission Error - Check HubSpot Token Scopes</option>');
                        } else {
                            formSelect.html('<option value="">❌ ' + response.data.substring(0, 50) + '...</option>');
                        }
                    } else {
                        formSelect.html('<option value="">No forms found</option>');
                    }
                }
            },
            error: function(xhr, status, error) {
    ...
    /**
     * Update HubSpot form options
     */
    function updateHubSpotFormOptions(forms) {
        const formSelect = $('#hubspot_form_select');
        let options = '<option value="">Select a HubSpot form...</option>';
        
        Object.entries(forms).forEach(function([formId, formName]) {
    ...
    /**
     * Load HubSpot form fields for field mapping
     */
    function loadHubSpotFormFields(formId) {
        // For HubSpot forms, we'll use a simple field mapping
        // The actual form fields will be determined by the form structure in HubSpot
        // We'll provide common form field options for mapping
        hubspotProperties = [
    ...
    function loadFormFields() {
        return new Promise((resolve, reject) => {
            $.ajax({
                ...
                type: 'POST',
                data: {
                    action: 'connect2form_hubspot_get_form_fields',
                    nonce: connect2formCFHubspot.nonce,
                    form_id: currentFormId
                },
                timeout: CONFIG.AJAX_TIMEOUT,
    ...
    function updateFieldMappingTable() {
        // Clean up orphaned mappings when we have form fields loaded
        if (formFields && formFields.length > 0 && Object.keys(currentMapping).length > 0) {
            const currentFieldIds = formFields.map(field => field.id);
            const savedFieldIds = Object.keys(currentMapping);
            const orphanedMappings = savedFieldIds.filter(id => !currentFieldIds.includes(id));
            
            if (orphanedMappings.length > 0) {
                orphanedMappings.forEach(id => delete currentMapping[id]);
            }
        }
    ...
    function handleClearMappings() {
        if (confirm(connect2formCFHubspot.strings.clearMappingsConfirm)) {
            currentMapping = {};
            updateFieldMappingTable();
    ...
    /**
     * Handle clean orphaned mappings
     */
    function handleCleanMappings() {
        const currentFieldIds = formFields.map(field => field.id);
        const savedFieldIds = Object.keys(currentMapping);
        const orphanedMappings = savedFieldIds.filter(id => !currentFieldIds.includes(id));
        
        if (orphanedMappings.length === 0) {
            showMessage('No orphaned mappings found', 'info');
            return;
        }
        
        if (confirm(`Found ${orphanedMappings.length} orphaned field mappings. Remove them?`)) {
            orphanedMappings.forEach(id => delete currentMapping[id]);
            updateFieldMappingTable();
            updateMappingCount();
            showMessage(`Cleaned up ${orphanedMappings.length} orphaned mappings`, 'success');
        }
    }
