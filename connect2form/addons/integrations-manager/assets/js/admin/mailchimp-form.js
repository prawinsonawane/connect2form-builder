...
(function($) {
    ...
    function loadFormFields() {
        ...
        $.ajax({
            url: connect2formCFMailchimp.ajaxUrl,
            type: 'POST',
            data: {
                action: 'connect2form_mailchimp_get_form_fields',
                form_id: currentFormId,
                nonce: connect2formCFMailchimp.nonce
            },
    ...
    function loadAudiences() {
        ...
        if (!apiKey) {
            ...
            return;
                ...
                type: 'POST',
                data: {
                    action: 'connect2form_get_mailchimp_audiences',
                    nonce: connect2formCFMailchimp.nonce,
                api_key: apiKey
            },
            timeout: CONFIG.AJAX_TIMEOUT,
    ...
    function getApiKey() {
        ...
        // Try connect2formCFMailchimp global settings
        if (window.connect2formCFMailchimp?.globalSettings?.api_key) {
            return window.connect2formCFMailchimp.globalSettings.api_key;
        }
    ...
    function populateAudienceSelect(audiences) {
        ...
        $select.empty().append('<option value="">' + (connect2formCFMailchimp.strings.selectAudience || 'Select an audience...') + '</option>');
    ...
    function autoSaveAudienceSelection(audienceId) {
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'connect2form_save_mailchimp_audience_selection',
                nonce: connect2formCFMailchimp.nonce,
                form_id: currentFormId,
    ...
    function loadMailchimpFieldsInternal(audienceId) {
        ...
        if (!window.connect2formCFMailchimp?.ajaxUrl) {
        ...
        $.ajax({
            url: window.connect2formCFMailchimp.ajaxUrl,
            type: 'POST',
            data: {
                action: 'connect2form_mailchimp_get_audience_merge_fields',
                audience_id: audienceId,
                nonce: window.connect2formCFMailchimp.nonce
            },
            timeout: CONFIG.AJAX_TIMEOUT,
            success: function(response) {
    ...
    function saveFormSettings() {
        ...
        // Collect form data
        const formData = {
            action: 'connect2form_save_mailchimp_form_settings',
            nonce: connect2formCFMailchimp.nonce,
            form_id: currentFormId,
