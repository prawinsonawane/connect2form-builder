...
class Connect2Form_Form_Renderer {
    ...
    public function enqueue_scripts($form_id = null) {
        ...
        wp_localize_script('connect2form-frontend', 'connect2formCF', array(
            ...
            'messages' => array(
                'required' => __('This field is required.', 'connect2form'),
                'email' => __('Please enter a valid email address.', 'connect2form'),
                'success' => __('Form submitted successfully!', 'connect2form'),
                'error' => __('An error occurred. Please try again.', 'connect2form')
            )
        ));
    }
    ...
    public function render_form($form_id) {
        ...
        if (!$form) {
            return '<div class="connect2form-error"><p>' . __('Form not found.', 'connect2form') . '</p></div>';
        }
        ...
        if (!empty($form_fields) && is_array($form_fields)) {
            ...
            $form_html .= '<p>' . __('No form fields found.', 'connect2form') . '</p>';
        }
        ...
        // Add submit button
        $submit_text = !empty($form_settings['submit_text']) ? $form_settings['submit_text'] : __('Submit', 'connect2form');
            ...
            switch ($field_type) {
                ...
                case 'select':
                    ...
                    <select id="<?php echo esc_attr($field_id); ?>"
                            ...
                            <?php if ($required): ?>required<?php endif; ?>
                            <?php if (!empty($field['css_class'])): ?>class="<?php echo esc_attr($field['css_class']); ?>"<?php endif; ?>>
                        <option value=""><?php echo esc_html($placeholder ?: __('Select an option', 'connect2form')); ?></option>
                        <?php
                ...
                case 'utm':
                    ...
                    foreach ($utm_params as $param) {
                        $param_value = isset($_GET[$param]) ? sanitize_text_field($_GET[$param]) : '';
                        ?>
                        <input type="hidden" 
                ...
                case 'captcha':
                    if (!empty($site_key)) {
                        ...
                        echo '<p>' . esc_html__('reCAPTCHA not configured. Please configure reCAPTCHA settings.', 'connect2form') . '</p>';
                    }
                    break;
                ...
                default:
                    ...
                    if (!empty($custom_field_html)) {
                        ...
                        echo '<p>' . esc_html(sprintf(__('Unknown field type: %s', 'connect2form'), $field_type)) . '</p>';
                    }
                    break;
            }
