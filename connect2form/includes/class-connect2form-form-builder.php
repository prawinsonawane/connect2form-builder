...
class Connect2Form_Form_Builder {
	...
	private function init_field_types() {
		...
		$this->field_types = array(
			...
			'textarea' => array(
				'label'    => __( 'Paragraph Text', 'connect2form' ),
			...
			'multiselect' => array(
				'label'    => __( 'Multiple Choice', 'connect2form' ),
				'icon'     => 'dashicons-list-view',
				'settings' => array( 'label', 'required', 'choices', 'layout' ),
			),
			'image_choice' => array(
				'label'    => __( 'Image Choice', 'connect2form' ),
			...
			'hidden' => array(
				'label'    => __( 'Hidden', 'connect2form' ),
				'icon'     => 'dashicons-hidden',
				'settings' => array( 'label', 'default_value' ),
			),
			'html' => array(
				'label'    => __( 'HTML', 'connect2form' ),
				'icon'     => 'dashicons-editor-code',
				'settings' => array( 'content' ),
			),
			'captcha' => array(
				'label'    => __( 'CAPTCHA', 'connect2form' ),
				'icon'     => 'dashicons-shield',
				'settings' => array( 'label', 'site_key', 'secret_key', 'theme' ),
			),
			'submit' => array(
				'label'    => __( 'Submit Button', 'connect2form' ),
	...
	private function render_setting_field( $setting, $value, $field_type ) {
		...
		switch ( $setting ) {
			...
			case 'layout':
				...
				$output .= '<option value="vertical" ' . selected( $value, 'vertical', false ) . '>' . __( 'Vertical', 'connect2form' ) . '</option>';
				$output .= '<option value="horizontal" ' . selected( $value, 'horizontal', false ) . '>' . __( 'Horizontal', 'connect2form' ) . '</option>';
				$output .= '</select>';
				break;
			...
			case 'theme':
				$output .= '<select name="settings[' . esc_attr( $setting ) . ']" class="widefat">';
				$output .= '<option value="light" ' . selected( $value, 'light', false ) . '>' . __( 'Light', 'connect2form' ) . '</option>';
				$output .= '<option value="dark" ' . selected( $value, 'dark', false ) . '>' . __( 'Dark', 'connect2form' ) . '</option>';
				$output .= '</select>';
				break;
		}
	...
	private function render_choices_setting( $choices ) {
		...
		$output .= '<button type="button" class="button add-choice">' . __( 'Add Choice', 'connect2form' ) . '</button>';
		$output .= '</div>';
		return $output;
	}
	...
	private function render_choice_row( $index, $choice ) {
		...
		$output .= '<input type="text" name="settings[choices][' . esc_attr( $index ) . '][label]" value="' . esc_attr( $choice['label'] ) . '" placeholder="' . __( 'Label', 'connect2form' ) . '" class="widefat">';
		$output .= '<input type="text" name="settings[choices][' . esc_attr( $index ) . '][value]" value="' . esc_attr( $choice['value'] ) . '" placeholder="' . __( 'Value', 'connect2form' ) . '" class="widefat">';
		$output .= '<button type="button" class="button remove-choice">' . __( 'Remove', 'connect2form' ) . '</button>';
		$output .= '</div>';
		return $output;
	}
	...
	private function get_setting_label( $setting ) {
		$labels = array(
			'label'         => __( 'Field Label', 'connect2form' ),
			'required'      => __( 'Required', 'connect2form' ),
			'placeholder'   => __( 'Placeholder', 'connect2form' ),
			'default_value' => __( 'Default Value', 'connect2form' ),
			'choices'       => __( 'Choices', 'connect2form' ),
			'content'       => __( 'HTML Content', 'connect2form' ),
			'rows'          => __( 'Number of Rows', 'connect2form' ),
			'min'           => __( 'Minimum Value', 'connect2form' ),
			'max'           => __( 'Maximum Value', 'connect2form' ),
			'step'          => __( 'Step', 'connect2form' ),
			'layout'        => __( 'Layout', 'connect2form' ),
			'date_format'   => __( 'Date Format', 'connect2form' ),
			'allowed_types' => __( 'Allowed File Types', 'connect2form' ),
			'max_size'      => __( 'Maximum File Size (MB)', 'connect2form' ),
			'max_files'     => __( 'Maximum Number of Files', 'connect2form' ),
			'site_key'      => __( 'reCAPTCHA Site Key', 'connect2form' ),
			'secret_key'    => __( 'reCAPTCHA Secret Key', 'connect2form' ),
			'theme'         => __( 'reCAPTCHA Theme', 'connect2form' ),
		);
