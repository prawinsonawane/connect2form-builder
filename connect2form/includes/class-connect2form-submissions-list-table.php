...
class Connect2Form_Submissions_List_Table extends WP_List_Table {
    ...
    public function get_columns() {
        $columns = array(
            ...
            'id'        => esc_html__('ID', 'connect2form'),
            'date'      => esc_html__('Date', 'connect2form')
        );

        // If no form_id is provided, show standard columns plus form info
        if (!$this->form_id) {
            $columns['form_id'] = esc_html__('Form', 'connect2form');
            $columns['actions'] = esc_html__('Actions', 'connect2form');
            return $columns;
        }
    ...
    public function column_default($item, $column_name) {
        ...
        if ($column_name === 'form_id') {
            ...
            return $form ? esc_html($form->form_title) : __('Unknown Form', 'connect2form');
        }
        ...
        if ($this->form_fields && is_array($this->form_fields)) {
            foreach ($this->form_fields as $field) {
                if ($field['id'] === $column_name) {
                    return '<em>' . esc_html__('Field not displayed (limit: 4 fields)', 'connect2form') . '</em>';
                }
            }
        }

        return '<em>' . esc_html__('No data', 'connect2form') . '</em>';
    }
    ...
    private function get_row_actions($item) {
        $actions = array(
            ...
            'delete' => sprintf(
                ...
                esc_html__('Delete', 'connect2form')
            )
        );
    ...
    private function format_field_value($field_id, $value) {
        ...
        switch ($field['type']) {
            ...
            case 'file_upload':
                ...
                return '<em>' . esc_html__('No file uploaded', 'connect2form') . '</em>';
            ...
            case 'date':
                ...
                return '<em>' . __('No date', 'connect2form') . '</em>';
            ...
            case 'utm':
                if (is_array($value)) {
                    ...
                    return !empty($utm_parts) ? implode('<br>', $utm_parts) : '<em>' . esc_html__('No UTM data', 'connect2form') . '</em>';
                }
                return esc_html($value);
            ...
            case 'textarea':
                ...
                return '<em>' . esc_html__('No content', 'connect2form') . '</em>';
    ...
    public function get_bulk_actions() {
        return array(
            'delete' => esc_html__('Delete', 'connect2form'),
            'export' => esc_html__('Export', 'connect2form')
        );
    }
    ...
    public function extra_tablenav( $which ) {
        ...
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared -- static forms list query for filter dropdown with trusted table identifier; no caching needed for admin filter dropdown
        $forms = $wpdb->get_results( "SELECT id, form_title FROM `{$table}` ORDER BY form_title" ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- validated identifier
        ...
        <div class="alignleft actions">
            <form method="get" action="<?php echo esc_url( $action_url ); ?>" id="connect2form-filter-form" style="display:inline-block;">
                ...
                <label for="form-filter" class="screen-reader-text"><?php esc_html_e( 'Filter by form', 'connect2form' ); ?></label>
                <select name="form_id" id="form-filter">
                    <option value=""><?php esc_html_e( 'All Forms', 'connect2form' ); ?></option>
                    <?php if ( ! empty( $forms ) ) : ?>
                ...
                <input type="submit" name="filter_submissions" value="<?php echo esc_attr__( 'Filter', 'connect2form' ); ?>" class="button" />
    
                <?php if ( $current_formid ) : ?>
                    <a href="<?php echo esc_url( $list_page_url ); ?>" class="button">
                        <?php esc_html_e( 'Clear Filter', 'connect2form' ); ?>
                    </a>
                <?php endif; ?>
            </form>
        </div>
        ...
        jQuery(function($){
            ...
            if ($filterForm.length) {
                ...
                $filterForm.on('submit', function(){
                    $('<div id="form-submitting" style="position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#fff;border:2px solid #0073aa;padding:20px;border-radius:5px;z-index:9999;box-shadow:0 0 10px rgba(0,0,0,.3);"><?php echo esc_js( __( 'Form is being submitted...', 'connect2form' ) ); ?></div>').appendTo('body');
                });
            }
        });
        </script>
    ...
    public function no_items() {
        if (!$this->form_id) {
            esc_html_e('No submissions found.', 'connect2form');
        } else {
            esc_html_e('No submissions found for the selected form.', 'connect2form');
        }
    }
} 
