...
class Connect2Form_Admin {
    ...
    public function render_debug_page() {
        ?>
        <div class="wrap">
            <h1><?php echo esc_html__('Connect2Form Debug', 'connect2form'); ?></h1>
            
            <div class="notice notice-info">
                <p><?php echo esc_html__('This debug page helps diagnose issues with the Connect2Form plugin and its integrations.', 'connect2form'); ?></p>
            </div>
            ...
            <div class="debug-section">
                <h2><?php echo esc_html__('1. Plugin Status', 'connect2form'); ?></h2>
                <?php
                echo '<p><span class="debug-status">' . esc_html__('Connect2Form Active:', 'connect2form') . '</span> ' . (class_exists('Connect2Form_Admin') ? esc_html__('Yes', 'connect2form') : esc_html__('No', 'connect2form')) . '</p>';
                echo '<p><span class="debug-status">' . esc_html__('Integrations Manager Active:', 'connect2form') . '</span> ' . (class_exists('Connect2Form\\Integrations\\Core\\Plugin') ? esc_html__('Yes', 'connect2form') : esc_html__('No', 'connect2form')) . '</p>';
                
                if (defined('CONNECT2FORM_VERSION')) {
                    echo '<p><span class="debug-status">' . esc_html__('Connect2Form Version:', 'connect2form') . '</span> ' . esc_html(CONNECT2FORM_VERSION) . '</p>';
                }
                
                if (defined('CONNECT2FORM_INTEGRATIONS_VERSION')) {
                    echo '<p><span class="debug-status">' . esc_html__('Integrations Version:', 'connect2form') . '</span> ' . esc_html(CONNECT2FORM_INTEGRATIONS_VERSION) . '</p>';
                }
                ?>
            </div>
            
            <div class="debug-section">
                <h2><?php echo esc_html__('2. File Existence Check', 'connect2form'); ?></h2>
                <?php
                $files_to_check = [
                    'connect2form.php',
                    'addons/integrations-manager/integrations-manager.php',
                    'addons/integrations-manager/assets/js/admin/mailchimp.js',
                    'addons/integrations-manager/src/Integrations/Mailchimp/MailchimpIntegration.php',
                    'addons/integrations-manager/src/Core/Services/LanguageManager.php'
                ];
                ...
                foreach ($files_to_check as $file) {
                    ...
                    $status_text = $exists ? esc_html__('EXISTS', 'connect2form') : esc_html__('MISSING', 'connect2form');
            ...
            <div class="debug-section">
                <h2><?php echo esc_html__('3. Integration Registry Test', 'connect2form'); ?></h2>
                <?php
                try {
                    if (class_exists('Connect2Form\\Integrations\\Core\\Plugin')) {
                        $plugin = Connect2Form\Integrations\Core\Plugin::getInstance();
                        echo '<p class="debug-success">' . esc_html__('Plugin instance created successfully', 'connect2form') . '</p>';
                        
                        if (method_exists($plugin, 'getRegistry')) {
                            $registry = $plugin->getRegistry();
                            echo '<p class="debug-success">' . esc_html__('Registry obtained successfully', 'connect2form') . '</p>';
                            
                            if ($registry && method_exists($registry, 'getAll')) {
                                $integrations = $registry->getAll();
                                echo '<p><span class="debug-status">' . esc_html__('Registered Integrations:', 'connect2form') . '</span> ' . esc_html(count($integrations)) . '</p>';
                                ...
                                foreach ($integrations as $id => $integration) {
                                    ...
                                    if ($id === 'mailchimp') {
                                        ...
                                        try {
                                            ...
                                            echo '<p class="debug-success">✓ ' . esc_html__('getAvailableActions() works', 'connect2form') . '</p>';
                                            ...
                                            echo '<p class="debug-error">✗ ' . esc_html__('getAvailableActions() failed:', 'connect2form') . ' ' . esc_html($e->getMessage()) . '</p>';
                                        }
                                        echo '</div>';
                                    }
                                }
                        ...
                        echo '<p class="debug-error">' . esc_html__('Plugin class not found', 'connect2form') . '</p>';
                    }
                } catch (Exception $e) {
                    echo '<p class="debug-error">' . esc_html__('Error:', 'connect2form') . ' ' . esc_html($e->getMessage()) . '</p>';
                }
                ?>
            </div>
            
            <div class="debug-section">
                <h2><?php echo esc_html__('4. AJAX Handlers Test', 'connect2form'); ?></h2>
                <?php
                global $wp_filter;
                ...
                foreach ($ajax_actions_to_check as $action) {
                    ...
                    $status_text = $registered ? esc_html__('REGISTERED', 'connect2form') : esc_html__('NOT REGISTERED', 'connect2form');
                        ...
                        foreach ($wp_filter[$action]->callbacks as $priority => $callbacks) {
                            foreach ($callbacks as $callback) {
                                if (is_array($callback['function'])) {
                                    ...
                                    echo '<p>- ' . esc_html($class) . '::' . esc_html($callback['function'][1]) . ' ' . esc_html__('(priority:', 'connect2form') . ' ' . esc_html($priority) . ')</p>';
                                }
                            }
                        }
                        echo '</div>';
            ...
            <div class="debug-section">
                <h2><?php echo esc_html__('5. Text Domain Test', 'connect2form'); ?></h2>
                <?php
                $domain_loaded = is_textdomain_loaded('connect2form');
                $status_class = $domain_loaded ? 'debug-success' : 'debug-warning';
                $status_text = $domain_loaded ? esc_html__('Yes', 'connect2form') : esc_html__('No', 'connect2form');
                echo '<p class="' . esc_attr($status_class) . '"><span class="debug-status">' . esc_html__('Text domain "connect2form" loaded:', 'connect2form') . '</span> ' . esc_html($status_text) . '</p>';
                
                $test_translation = __('Test String', 'connect2form');
                echo '<p><span class="debug-status">' . esc_html__('Test translation:', 'connect2form') . '</span> ' . esc_html($test_translation) . '</p>';
                ?>
            </div>
            
            <div class="debug-section">
                <h2><?php echo esc_html__('6. URL Tests', 'connect2form'); ?></h2>
                <?php
                echo '<p><span class="debug-status">' . esc_html__('Integrations Page URL:', 'connect2form') . '</span> <a href="' . esc_url(admin_url('admin.php?page=connect2form-integrations')) . '" target="_blank">' . esc_html(admin_url('admin.php?page=connect2form-integrations')) . '</a></p>';
                echo '<p><span class="debug-status">' . esc_html__('Mailchimp Settings URL:', 'connect2form') . '</span> <a href="' . esc_url(admin_url('admin.php?page=connect2form-integrations&tab=settings&integration=mailchimp')) . '" target="_blank">' . esc_html(admin_url('admin.php?page=connect2form-integrations&tab=settings&integration=mailchimp')) . '</a></p>';
                ?>
            </div>
            
            <div class="debug-section">
                <h2><?php echo esc_html__('7. WordPress Environment', 'connect2form'); ?></h2>
                <?php
                echo '<p><span class="debug-status">' . esc_html__('WordPress Version:', 'connect2form') . '</span> ' . esc_html(get_bloginfo('version')) . '</p>';
                echo '<p><span class="debug-status">' . esc_html__('PHP Version:', 'connect2form') . '</span> ' . esc_html(PHP_VERSION) . '</p>';
                echo '<p><span class="debug-status">' . esc_html__('Debug Mode:', 'connect2form') . '</span> ' . (WP_DEBUG ? esc_html__('Enabled', 'connect2form') : esc_html__('Disabled', 'connect2form')) . '</p>';
                echo '<p><span class="debug-status">' . esc_html__('Current User:', 'connect2form') . '</span> ' . esc_html(wp_get_current_user()->user_login) . '</p>';
                echo '<p><span class="debug-status">' . esc_html__('Current Time:', 'connect2form') . '</span> ' . esc_html(current_time('mysql')) . '</p>';
                ?>
            </div>
            
            <div class="debug-section">
                <h2><?php echo esc_html__('8. JavaScript Console Test', 'connect2form'); ?></h2>
                <p><?php echo esc_html__('Check the browser console (F12) for JavaScript debug information.', 'connect2form'); ?></p>
                <script>
    ...
    public function enqueue_scripts( $hook ) {
        ...
        wp_localize_script( 'connect2form-form-builder', 'connect2formCF', array(
            ...
            'i18n' => array(
                'confirmDelete' => __( 'Are you sure you want to delete this field?', 'connect2form' ),
                'fieldRequired' => __( 'This field is required', 'connect2form' ),
                'saveSuccess' => __( 'Form saved successfully', 'connect2form' ),
                'saveError' => __( 'Error saving form', 'connect2form' ),
            ),
        ) );
    ...
    public function render_forms_list_page() {
        ...
        ?>
        <div class="wrap">
            <h1 class="wp-heading-inline"><?php esc_html_e( 'Contact Forms', 'connect2form' ); ?></h1>
            <a href="<?php echo esc_url( admin_url( 'admin.php?page=connect2form-new-form' ) ); ?>" class="page-title-action"><?php esc_html_e( 'Add New', 'connect2form' ); ?></a>

            <hr class="wp-header-end">

            <form method="post">
                <?php
                $forms_table->search_box( __( 'Search Forms', 'connect2form' ), 'search_id' );
                $forms_table->display();
                ?>
            </form>
        </div>
        ...
        wp_localize_script( 'connect2form-admin', 'connect2formCF', array(
            ...
            'submissionBulkDeleteNonce' => wp_create_nonce( 'connect2form_bulk_delete_submissions' ),
            'submissionBulkExportNonce' => wp_create_nonce( 'connect2form_bulk_export_submissions' ),
            'confirmDelete' => __( 'Are you sure you want to delete this form?', 'connect2form' ),
            'confirmDeleteSubmission' => __( 'Are you sure you want to delete this submission?', 'connect2form' ),
            'copiedText' => __( 'Copied!', 'connect2form' ),
                        ...
                        <div class="form-builder-fields-layout">
                            ...
                            <div class="form-fields-sidebar">
                                <div class="field-types">
                                    <h3><?php echo esc_html__('Field Types', 'connect2form'); ?></h3>
                                    <div class="field-type-list">
                                        <?php
                                        $field_types = array(
                                            'text' => array(
                                                'label' => __('Textbox', 'connect2form'),
                                                'icon' => 'dashicons-editor-textcolor',
                                                'settings' => array('label', 'placeholder', 'required', 'validation')
                                            ),
                                            'textarea' => array(
                                                'label' => __('Textarea', 'connect2form'),
                                            ...
                                            /* 'number' => array(
                                                'label' => __('Number', 'connect2form'),
                                                'icon' => 'dashicons-calculator'
                                            ), */
                                            'date' => array(
                                                'label' => __('Date', 'connect2form'),
                                                'icon' => 'dashicons-calendar-alt'
                                            ),
                                            'phone' => array(
                                                'label' => __('Phone Number', 'connect2form'),
                                                'icon' => 'dashicons-phone'
                                            ),
                                            'select' => array(
                                                'label' => __('Drop Down', 'connect2form'),
                                                'icon' => 'dashicons-arrow-down-alt2'
                                            ),
                                            'radio' => array(
                                                'label' => __('Radio Buttons', 'connect2form'),
                                                'icon' => 'dashicons-marker'
                                            ),
                                            'checkbox' => array(
                                                'label' => __('Checkboxes', 'connect2form'),
                                                'icon' => 'dashicons-yes'
                                            ),
                                            'file' => array(
                                                'label' => __('File Upload', 'connect2form'),
                                                'icon' => 'dashicons-upload'
                                            ),
                                            'utm' => array(
                                                'label' => __('UTM Tracking', 'connect2form'),
                                                'icon' => 'dashicons-chart-line'
                                            ),
                                            'captcha' => array(
                                                'label' => __('reCAPTCHA', 'connect2form'),
                        ...
                        <div class="messages-settings">
                            ...
                            <div class="message-setting">
                                ...
                                <select id="redirect-type" class="widefat">
                                    <option value="message" <?php selected($this->get_form_setting($form, 'redirect_type', 'message'), 'message'); ?>><?php echo esc_html__('Show Thank You Message', 'connect2form'); ?></option>
                                    <option value="redirect" <?php selected($this->get_form_setting($form, 'redirect_type', 'message'), 'redirect'); ?>><?php echo esc_html__('Redirect to URL', 'connect2form'); ?></option>
                                </select>
...
?>
    ...
    public function render_submissions_page() {
        ...
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('Form Submissions', 'connect2form'); ?></h1>
            <hr class="wp-header-end">
                ...
                if ($form): ?>
                    <div class="notice notice-info">
                        <p><?php echo wp_kses_post(sprintf(
                            ...
                            __('Showing submissions for form: <strong>%s</strong>', 'connect2form'), 
            ...
            <?php else: ?>
                <div class="notice notice-info">
                    <p><?php echo esc_html__('Showing all form submissions. Use the filter below to view submissions for a specific form.', 'connect2form'); ?></p>
                </div>
            <?php endif; ?>

            <div class="notice notice-warning">
                <p><?php echo wp_kses_post(__('&lt;strong&gt;Note:&lt;/strong&gt; The submissions table displays only the first 4 form fields for better readability. To view all field data, click on the "View" action for individual submissions.', 'connect2form')); ?></p>
            </div>

            <form method="post">
                <?php wp_nonce_field('bulk-submissions'); ?>
                <?php
                $submissions_table->search_box(__('Search Submissions', 'connect2form'), 'search_id');
    ...
    private function render_submission_view($submission_id) {
        ...
        if (!$submission) {
            wp_die(esc_html__('Submission not found.', 'connect2form'));
        }

        // Get form data
        $form = $this->get_form($submission->form_id);
        if (!$form) {
            wp_die(esc_html__('Form not found.', 'connect2form'));
        }
        ...
        <div class="wrap">
            <h1 class="wp-heading-inline">
                <?php echo esc_html(sprintf(
                    ...
                    __('Submission #%d', 'connect2form'), 
                    $submission_id
                )); ?>
            </h1>
                <a href="<?php echo esc_url(admin_url('admin.php?page=connect2form-submissions')); ?>" class="page-title-action">
                <?php esc_html_e('← Back to Submissions', 'connect2form'); ?>
            </a>
            <hr class="wp-header-end">

            <div class="submission-details">
                <div class="submission-meta">
                    <h2><?php echo esc_html__('Submission Details', 'connect2form'); ?></h2>
                    <table class="form-table">
                        <tr>
                            <th scope="row"><?php echo esc_html__('Form', 'connect2form'); ?></th>
                            <td><?php echo esc_html($form->form_title); ?></td>
                        </tr>
                        <tr>
                            <th scope="row"><?php echo esc_html__('Submitted', 'connect2form'); ?></th>
                            <td><?php echo esc_html(date_i18n(get_option('date_format') . ' ' . get_option('time_format'), strtotime($submission->created_at))); ?></td>
                        </tr>
                        <tr>
                            <th scope="row"><?php echo esc_html__('IP Address', 'connect2form'); ?></th>
                            <td><?php echo esc_html($submission->ip_address ?? __('N/A', 'connect2form')); ?></td>
                        </tr>
                        <tr>
                            <th scope="row"><?php echo esc_html__('User Agent', 'connect2form'); ?></th>
                            <td><?php echo esc_html($submission->user_agent ?? __('N/A', 'connect2form')); ?></td>
                        </tr>
                    </table>
                </div>

                <div class="submission-fields">
                    <h2><?php echo esc_html__('Form Data', 'connect2form'); ?></h2>
                    <table class="form-table">
                        <?php
                        if (!empty($form->fields) && is_array($form->fields)) {
                            ...
                            echo '<tr><td colspan="2">' . esc_html__('No form fields found or form fields not properly configured.', 'connect2form') . '</td></tr>';
                        }
                        ?>
                    </table>
                </div>
                ...
                <div class="submission-actions">
                    <h2><?php echo esc_html__('Actions', 'connect2form'); ?></h2>
                    <p>
                        ...
                        <a href="<?php echo esc_url(admin_url('admin.php?page=connect2form-submissions&action=delete&id=' . $submission_id)); ?>" 
                           ...
                           onclick="return confirm('<?php echo esc_js(__('Are you sure you want to delete this submission?', 'connect2form')); ?>');">
                            <?php esc_html_e('Delete Submission', 'connect2form'); ?>
                        </a>
                    </p>
                </div>
            </div>
    ...
    private function format_submission_field_value($field, $value) {
        if (empty($value)) {
            return '<em>' . __('No value', 'connect2form') . '</em>';
        }
        ...
        switch ($field['type']) {
            ...
            case 'utm':
                if (is_array($value)) {
                    ...
                    return !empty($utm_parts) ? implode('<br>', $utm_parts) : '<em>' . __('No UTM data captured', 'connect2form') . '</em>';
                }
    ...
    public function ajax_save_form() {
        try {
            ...
            if ( ! check_ajax_referer( 'connect2form_nonce', 'nonce', false ) ) {
                throw new Exception( esc_html__( 'Invalid security token.', 'connect2form' ) );
            }
            ...
            if ( ! $table_exists ) {
                ...
                throw new Exception( esc_html__( 'Database table is missing. Please deactivate and reactivate the plugin.', 'connect2form' ) );
            }

            // Check permissions - allow administrators.
            if ( ! current_user_can( 'manage_options' ) ) {
                throw new Exception( esc_html__( 'Permission denied.', 'connect2form' ) );
            }
            ...
            if ( json_last_error() !== JSON_ERROR_NONE ) {
                ...
                throw new Exception( esc_html__( 'Invalid form fields data.', 'connect2form' ) );
            }
            ...
            if ( json_last_error() !== JSON_ERROR_NONE ) {
                ...
                throw new Exception( esc_html__( 'Invalid form settings data.', 'connect2form' ) );
            }
    ...
    public function ajax_delete_form() {
      ...
      $form_id = isset( $_POST['form_id'] ) ? absint( $_POST['form_id'] ) : 0;
        ...
        $response_data = array(
            'message' => __( 'Form deleted successfully.', 'connect2form' ),
            'form_id' => $form_id,
        );
    ...
    public function ajax_bulk_delete_forms() {
        ...
        foreach ( $form_ids as $form_id ) {
            ...
            if ( class_exists( '\Connect2Form\Integrations\Core\Services\ServiceManager' ) ) {
                ...
                // Get form data before deletion for hooks.
                // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- fallback form query for deletion hook; service layer preferred but this is a fallback
                $form_data = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $table_name WHERE id = %d", $form_id ) );
        ...
        if ( $deleted_count > 0 ) {
            $message = sprintf(
                ...
                _n( '%d form deleted successfully.', '%d forms deleted successfully.', $deleted_count, 'connect2form' ),
                $deleted_count
            );
    ...
    public function render_form_preview($form_id) {
        ...
        <body>
            <div class="preview-header">
                ...
                <p><?php echo esc_html__('Form Preview', 'connect2form'); ?></p>
            </div>
            ...
            <div class="preview-actions">
                <a href="<?php echo esc_url(admin_url('admin.php?page=connect2form&action=edit&id=' . $form_id)); ?>" class="button button-primary">
                    <?php esc_html_e('Edit Form', 'connect2form'); ?>
                </a>
                <a href="<?php echo esc_url(admin_url('admin.php?page=connect2form')); ?>" class="button">
                    <?php esc_html_e('Back to Forms', 'connect2form'); ?>
                </a>
                ...
                <a href="<?php echo esc_url( add_query_arg( 'preview', '1', esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ?? '' ) ) ) ); ?>" class="button">
                    <?php esc_html_e('Preview', 'connect2form'); ?>
                </a>
                <button type="button" class="button" onclick="window.close();">
                    <?php esc_html_e('Close Preview', 'connect2form'); ?>
                </button>
            </div>
