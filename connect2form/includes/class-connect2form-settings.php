...
class Connect2Form_Settings {
	...
	public function register_settings() {
		...
		add_settings_section(
			...
			__( 'Anti-Spam Protection', 'connect2form' ),
		...
		add_settings_section(
			...
			__( 'Google reCAPTCHA Settings', 'connect2form' ),
		...
		add_settings_field(
			'enable_honeypot',
			__( 'Enable Anti-spam Honeypot', 'connect2form' ),
		...
		add_settings_field(
			'honeypot_action',
			__( 'Honeypot Action', 'connect2form' ),
		...
		add_settings_field(
			'recaptcha_type',
			__( 'reCAPTCHA Type', 'connect2form' ),
	...
	public function render_antispam_section() {
		printf( '<p>%s</p>', esc_html__( 'Configure anti-spam protection settings for your forms.', 'connect2form' ) );
	}
	...
	public function render_recaptcha_section() {
		printf( '<p>%s</p>', esc_html__( 'Configure Google reCAPTCHA settings. You can get your Site Key and Secret Key from the Google reCAPTCHA admin console.', 'connect2form' ) );
		printf( '<p><a href="%s" target="_blank" rel="noopener noreferrer">%s</a></p>', esc_url( 'https://www.google.com/recaptcha/admin' ), esc_html__( 'Get reCAPTCHA Keys', 'connect2form' ) );
	}
	...
	public function render_honeypot_field() {
		...
		<label>
			...
			<?php esc_html_e( 'Enable honeypot spam protection', 'connect2form' ); ?>
		</label>
		<p class="description">
			<?php esc_html_e( 'The honeypot technique adds an invisible field to your forms that should be left empty. Since automated bots typically fill all fields, this helps identify and block spam submissions.', 'connect2form' ); ?>
		</p>
		<?php
	}
	...
	public function render_honeypot_action_field() {
		...
		<select name="connect2form_antispam_settings[honeypot_action]">
			<option value="mark_spam" <?php selected( $action, 'mark_spam' ); ?>>
				<?php esc_html_e( 'Create an entry and mark it as spam', 'connect2form' ); ?>
			</option>
			<option value="discard" <?php selected( $action, 'discard' ); ?>>
				<?php esc_html_e( 'Do not create an entry', 'connect2form' ); ?>
			</option>
		</select>
		<?php
	}
	...
	public function render_recaptcha_type_field() {
		...
		<select name="connect2form_recaptcha_settings[recaptcha_type]">
			<option value="v2_checkbox" <?php selected( $type, 'v2_checkbox' ); ?>>
				<?php esc_html_e( 'reCAPTCHA v2 Checkbox', 'connect2form' ); ?>
			</option>
			<option value="v2_invisible" <?php selected( $type, 'v2_invisible' ); ?>>
				<?php esc_html_e( 'reCAPTCHA v2 Invisible', 'connect2form' ); ?>
			</option>
			<option value="v3" <?php selected( $type, 'v3' ); ?>>
				<?php esc_html_e( 'reCAPTCHA v3', 'connect2form' ); ?>
			</option>
		</select>
		<?php
	}
