...
class Connect2Form_Accessibility {
	...
	public function enhance_form_accessibility( $html, $form ) {
		...
		$form_title = esc_attr( $form->form_title ?? __( 'Contact Form', 'connect2form' ) );
		$html       = str_replace(
			'<form',
	...
	private function wrap_in_fieldset( $matches ) {
		...
		$legend = $label_matches[1] ?? __( 'Form Field Group', 'connect2form' );
	...
	private function ensure_input_has_label( $matches ) {
		...
		if ( strpos( $input, 'aria-label' ) === false ) {
			$input = str_replace( '>', ' aria-label="' . __( 'Form Input Field', 'connect2form' ) . '">', $input );
		}
			...
			document.addEventListener('DOMContentLoaded', function() {
				...
				forms.forEach(function(form) {
					...
					form.addEventListener('connect2form:success', function(e) {
						...
						if (announcement) {
							announcement.textContent = '<?php echo esc_js( __( 'Form submitted successfully', 'connect2form' ) ); ?>';
						}
					});
				});
	...
	public function add_admin_keyboard_navigation() {
		...
		(function() {
			...
			document.addEventListener('DOMContentLoaded', function() {
				...
				skipLink.textContent = '<?php echo esc_js( __( 'Skip to main content', 'connect2form' ) ); ?>';
	...
	public function add_screen_reader_support( $text, $context ) {
		$screen_reader_texts = array(
			'required_field' => __( 'Required field', 'connect2form' ),
			'error_field'    => __( 'Error in field', 'connect2form' ),
			'loading'        => __( 'Loading, please wait', 'connect2form' ),
			'form_submitted' => __( 'Form submitted successfully', 'connect2form' ),
			'form_error'     => __( 'Form submission failed, please check for errors', 'connect2form' ),
			'file_selected'  => __( 'File selected', 'connect2form' ),
			'file_removed'   => __( 'File removed', 'connect2form' ),
		);
	...
	public function enqueue_accessibility_assets() {
		...
		wp_localize_script( 'connect2form-accessibility', 'connect2formA11y', array(
			'strings' => array(
				'required' => __( 'Required field', 'connect2form' ),
				'error'    => __( 'Error in field', 'connect2form' ),
				'loading'  => __( 'Loading', 'connect2form' ),
				'success'  => __( 'Success', 'connect2form' ),
			),
		) );
	}
}
