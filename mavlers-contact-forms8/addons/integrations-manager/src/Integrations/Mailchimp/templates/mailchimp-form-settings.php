...
echo '<strong>Debug: Mailchimp Template Loading</strong><br>';
echo 'Form ID: ' . esc_html($form_id) . '<br>';
echo 'GET[id]: ' . (isset($_GET['id']) ? esc_html($_GET['id']) : 'not set') . '<br>';
echo 'mailchimp_form_id: ' . (isset($mailchimp_form_id) ? esc_html($mailchimp_form_id) : 'not set') . '<br>';
echo 'Global mavlers_cf_current_form_id: ' . (isset($GLOBALS['mavlers_cf_current_form_id']) ? esc_html($GLOBALS['mavlers_cf_current_form_id']) : 'not set') . '<br>';
echo '</div>';
...
try {
    ...
    // Also auto-enable if we have any meaningful settings
    if (isset($form_settings['audience_id']) && !empty($form_settings['audience_id'])) {
        $form_settings['enabled'] = true;
    }
    
    // Also auto-enable if we have field mappings
    if (isset($form_settings['field_mapping']) && !empty($form_settings['field_mapping'])) {
        $form_settings['enabled'] = true;
    }
    ...
    echo '<div style="background: #e8f5e8; padding: 10px; margin: 10px 0; border: 1px solid #4caf50; font-family: monospace; font-size: 12px;">';
    echo '<strong>Debug: Mailchimp Settings</strong><br>';
    echo 'Is Configured: ' . ($is_configured ? 'true' : 'false') . '<br>';
    echo 'Form Settings: ' . esc_html(print_r($form_settings, true)) . '<br>';
    ...
    echo '<div style="background: #ffebee; padding: 10px; margin: 10px 0; border: 1px solid #f44336; font-family: monospace; font-size: 12px;">';
    echo '<strong>Debug: Mailchimp Error</strong><br>';
        ...
        <div class="integration-form-settings">
            ...
            <div class="integration-settings" id="mailchimp_settings">
                <form id="mailchimp-form-settings">
                    ...
                    <div class="setting-row field-mapping-section" id="field-mapping-section" style="display: none;">
                        <label class="setting-label">
                            <?php _e('Field Mapping', 'mavlers-contact-forms'); ?>
                            <span class="setting-badge">Easy Mapping</span>
                        </label>
                        <div class="setting-control">
                            <div class="field-mapping-instructions">
                            ...
                            <div class="field-mapping-table-container">
                                <table class="field-mapping-table">
                                    <thead>
                                        <tr>
                                            <th class="form-field-column"><?php _e('Your Form Fields', 'mavlers-contact-forms'); ?></th>
                                            <th class="arrow-column"></th>
                                            <th class="mailchimp-field-column"><?php _e('Mailchimp Fields', 'mavlers-contact-forms'); ?></th>
                                            <th class="status-column"><?php _e('Status', 'mavlers-contact-forms'); ?></th>
                                        </tr>
                                    </thead>
                            ...
                            <div class="field-mapping-actions">
                                ...
                                <button type="button" id="clear-mappings" class="button button-secondary">
                                    <span class="dashicons dashicons-dismiss"></span>
                                    <?php _e('Clear All Mappings', 'mavlers-contact-forms'); ?>
                                </button>
                                <div class="mapping-status" id="mapping-status">
                                    <span class="mapping-count">0 fields mapped</span>
                                </div>
                            </div>
                        </div>
                        <p class="setting-help">
...
// Debug output
console.log('📋 Mailchimp Template: Settings loaded:', window.mailchimpFormSettings);
console.log('📋 Mailchimp Template: Form ID:', window.mailchimpFormId);
console.log('📋 Mailchimp Template: Global settings:', window.mailchimpGlobalSettings);
</script> 