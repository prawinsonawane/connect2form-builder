...
class CustomFieldsManager {
    ...
    public function __construct($mailchimp_integration = null) {
        error_log("Mailchimp CustomFieldsManager: Constructor called");
        error_log("Mailchimp CustomFieldsManager: Integration instance: " . ($mailchimp_integration ? 'provided' : 'null'));
        
        $this->mailchimp_integration = $mailchimp_integration;
        $this->init_hooks();
        
        error_log("Mailchimp CustomFieldsManager: Constructor completed");
    }
    ...
    public function get_merge_fields($audience_id, $force_refresh = false) {
        error_log("Mailchimp CustomFieldsManager: get_merge_fields method called");
        error_log("Mailchimp CustomFieldsManager: Getting merge fields for audience: {$audience_id}");
        error_log("Mailchimp CustomFieldsManager: Force refresh: " . ($force_refresh ? 'true' : 'false'));
        
        if (empty($audience_id)) {
            error_log("Mailchimp CustomFieldsManager: Audience ID is empty");
        ...
        if (!$force_refresh) {
            ...
            if ($cached !== false) {
                error_log("Mailchimp CustomFieldsManager: Using cached merge fields");
                return array('success' => true, 'data' => $cached);
            }
        }
        
        if (!$this->mailchimp_integration) {
            error_log("Mailchimp CustomFieldsManager: Mailchimp integration instance is null");
        ...
        if (!$this->mailchimp_integration->is_globally_connected()) {
            error_log("Mailchimp CustomFieldsManager: Integration not globally connected");
            return array('success' => false, 'message' => 'Mailchimp not connected');
        }
        
        $global_settings = $this->mailchimp_integration->get_global_settings();
        error_log("Mailchimp CustomFieldsManager: Global settings: " . print_r($global_settings, true));
        
        if (empty($global_settings['api_key'])) {
            error_log("Mailchimp CustomFieldsManager: No API key found in global settings");
            return array('success' => false, 'message' => 'Mailchimp API key not configured');
        }
        
        $api_key = $global_settings['api_key'];
        error_log("Mailchimp CustomFieldsManager: API key found: " . substr($api_key, 0, 10) . "...");
        
        $dc = $this->extract_datacenter($api_key);
        error_log("Mailchimp CustomFieldsManager: Extracted datacenter: " . ($dc ?: 'null'));
        
        if (empty($dc)) {
            error_log("Mailchimp CustomFieldsManager: Could not extract datacenter from API key");
            return array('success' => false, 'message' => 'Invalid API key format');
        }
        
        error_log("Mailchimp CustomFieldsManager: Making API request to datacenter: {$dc}");
        error_log("Mailchimp CustomFieldsManager: API endpoint: /lists/{$audience_id}/merge-fields?count=100");
        ...
        error_log("Mailchimp CustomFieldsManager: API response: " . print_r($response, true));
        
        if (!$response) {
            error_log("Mailchimp CustomFieldsManager: No response received from API");
            return array('success' => false, 'message' => 'No response from Mailchimp API');
        }
        
        if (isset($response['status']) && $response['status'] >= 400) {
            error_log("Mailchimp CustomFieldsManager: API request failed with status: " . $response['status']);
            $error_message = $response['detail'] ?? $response['title'] ?? 'Failed to fetch merge fields';
            error_log("Mailchimp CustomFieldsManager: Error message: " . $error_message);
            return array(
                'success' => false, 
        ...
        if (!isset($response['merge_fields'])) {
            error_log("Mailchimp CustomFieldsManager: No merge_fields key in response");
            return array('success' => false, 'message' => 'Invalid response format from Mailchimp API');
        }
        
        $merge_fields = $response['merge_fields'] ?? array();
        error_log("Mailchimp CustomFieldsManager: Raw merge fields count: " . count($merge_fields));
        
        if (empty($merge_fields)) {
            error_log("Mailchimp CustomFieldsManager: No merge fields found in response");
            return array('success' => false, 'message' => 'No merge fields found for this audience');
        }
        
        // Process and enhance merge fields
        $processed_fields = $this->process_merge_fields($merge_fields);
        error_log("Mailchimp CustomFieldsManager: Processed fields: " . print_r($processed_fields, true));
        ...
        switch ($type) {
            ...
            default:
                if (!empty($options['size'])) {
                    ...
                    $rules['message'] = sprintf(__('Text must be %d characters or less', 'mavlers-cf'), $rules['max_length']);
                }
                break;
        }
    ...
    private function extract_datacenter($api_key) {
        error_log("Mailchimp CustomFieldsManager: Extracting datacenter from API key");
        
        if (empty($api_key)) {
            error_log("Mailchimp CustomFieldsManager: API key is empty");
            return '';
        }
        
        // Check if API key has the expected format (32 chars + hyphen + datacenter)
        if (!preg_match('/^[a-f0-9]{32}-[a-z0-9]+$/', $api_key)) {
            error_log("Mailchimp CustomFieldsManager: API key format is invalid");
            return '';
        }
        ...
        if (strpos($api_key, '-') !== false) {
            ...
            error_log("Mailchimp CustomFieldsManager: Extracted datacenter: {$dc}");
        } else {
            error_log("Mailchimp CustomFieldsManager: No hyphen found in API key");
        }
    ...
    private function make_mailchimp_request($method, $endpoint, $api_key, $dc, $data = array()) {
        ...
        error_log("Mailchimp CustomFieldsManager: Making request to URL: {$url}");
        error_log("Mailchimp CustomFieldsManager: Method: {$method}");
        error_log("Mailchimp CustomFieldsManager: Datacenter: {$dc}");
        ...
        if (!empty($data) && in_array($method, array('POST', 'PUT', 'PATCH'))) {
            ...
            error_log("Mailchimp CustomFieldsManager: Request body: " . json_encode($data));
        }
        
        error_log("Mailchimp CustomFieldsManager: Request args: " . print_r($args, true));
        
        $response = wp_remote_request($url, $args);
        
        if (is_wp_error($response)) {
            error_log("Mailchimp CustomFieldsManager: WP Error: " . $response->get_error_message());
        ...
        error_log("Mailchimp CustomFieldsManager: Response code: {$response_code}");
        error_log("Mailchimp CustomFieldsManager: Response body: {$body}");
        
        $decoded = json_decode($body, true);
        
        error_log("Mailchimp CustomFieldsManager: Decoded response: " . print_r($decoded, true));
