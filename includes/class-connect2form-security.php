...
class Connect2Form_Security {
    ...
    private function is_plugin_page() {
        ...
        return strpos($screen->id, 'connect2form-builder') !== false;
    }
    ...
    public function validate_nonce() {
        ...
        if (!wp_verify_nonce($nonce, $action)) {
            wp_send_json_error(array(
                'message' => __('Security verification failed. Please refresh the page and try again.', 'connect2form-builder'),
                'code' => 'invalid_nonce'
            ));
        }
    ...
    public static function check_capability($capability = 'manage_options', $context = '') {
        if (!current_user_can($capability)) {
            $message = sprintf(
                ...
                __('You do not have permission to %1$s. Required capability: %2$s', 'connect2form-builder'),
                $context,
                $capability
            );
    ...
    public static function validate_ajax_request( string $nonce_action, string $capability = 'manage_options' ): void {
        ...
        if ( ! wp_doing_ajax() ) {
            wp_send_json_error(
                array( 'message' => esc_html__('This endpoint can only be called via AJAX.', 'connect2form-builder') ),
                400
            );
        }
        ...
        if ( false === check_ajax_referer( $nonce_action, 'nonce', false ) ) {
            wp_send_json_error(
                array( 'message' => esc_html__('Security verification failed. Please refresh and try again.', 'connect2form-builder') ),
                403
            );
        }
        ...
        if ( ! current_user_can( $capability ) ) {
            wp_send_json_error(
                array( 'message' => esc_html__('You are not allowed to perform this action.', 'connect2form-builder') ),
                403
            );
        }
    }
