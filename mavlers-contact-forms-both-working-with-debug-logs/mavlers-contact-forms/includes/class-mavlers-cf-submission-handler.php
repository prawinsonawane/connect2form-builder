...
class Mavlers_CF_Submission_Handler {
    ...
    public function handle_submission() {
        try {
            // Add debugging
            error_log('Mavlers CF: Starting submission handler...');
            ...
            error_log('Mavlers CF: Processing form ID: ' . $form_id);
            ...
            if (!$form) {
                error_log('Mavlers CF: Form not found in database for ID: ' . $form_id);
                wp_send_json_error(__('Form not found', 'mavlers-contact-forms'));
            }

            error_log('Mavlers CF: Form found, proceeding with validation...');
            ...
            error_log('Mavlers CF: Validation passed, processing files...');
            ...
            error_log('Mavlers CF: Files processed, preparing submission data...');
            ...
            error_log('Mavlers CF: Saving submission to database...');
            ...
            if (!$table_exists) {
                error_log('Mavlers CF: Database table does not exist: ' . $table_name);
                wp_send_json_error(__('Database error: submissions table not found. Please check plugin installation.', 'mavlers-contact-forms'));
            }
            ...
            if ($result === false) {
                error_log('Mavlers CF: Database insert failed: ' . $wpdb->last_error);
                wp_send_json_error(__('Failed to save submission: ' . $wpdb->last_error, 'mavlers-contact-forms'));
            }

            $submission_id = $wpdb->insert_id;
            error_log('Mavlers CF: Submission saved with ID: ' . $submission_id);

            // Action hook after saving submission
            do_action('mavlers_cf_after_save_submission', $submission_id, $db_submission_data, $submission_data, $form);

            // Process legacy integrations (old hardcoded system) - DISABLED
            // User prefers new addon system only
            // if (class_exists('Mavlers_CF_Integrations')) {
            //     $integrations = new Mavlers_CF_Integrations();
            //     $form_data = array(
            //         'form_id' => $form_id,
            ...
            error_log('Mavlers CF: Triggering addon integrations...');
            ...
            do_action('mavlers_cf_after_submission', $submission_id, $addon_form_data);

            error_log('Mavlers CF: Addon integrations completed, processing email notifications...');
            ...
            error_log('Mavlers CF: Submission handler encountered an error: ' . $e->getMessage());
            wp_send_json_error(__('An unexpected error occurred. Please try again later.', 'mavlers-contact-forms'));
        }
    }
