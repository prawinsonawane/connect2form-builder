...
class HubspotIntegration extends AbstractIntegration {
    ...
    public function init_components() {
        ...
        try {
            ...
            error_log("HubspotIntegration: Registered mavlers_cf_after_submission hook");
            ...
            add_action('mavlers_cf_after_submission', [$this, 'handle_form_submission'], 10, 2);
            error_log("HubspotIntegration: Registered mavlers_cf_after_submission hook (fallback)");
        }
    }
    ...
    public function register_ajax_handlers() {
        ...
        // Add a temporary form settings save handler without nonce for debugging
        add_action('wp_ajax_hubspot_save_form_settings_debug', [$this, 'ajax_save_form_settings_debug']);
        error_log("HubSpot: Registered wp_ajax_hubspot_save_form_settings_debug");
        
        // Add submission test handler
        add_action('wp_ajax_hubspot_test_submission', [$this, 'ajax_test_submission']);
        error_log("HubSpot: Registered wp_ajax_hubspot_test_submission");
        ...
        error_log("HubSpot: Registered field mapping, custom objects, and object properties AJAX handlers");
    ...
    private function process_submission_immediate(int $submission_id, array $form_data, array $settings): array {
        ...
        if (!empty($settings['enabled']) && $settings['object_type'] === 'contacts') {
            error_log("HubspotIntegration: Creating/updating contact with settings: " . print_r($settings, true));
            ...
            error_log("HubspotIntegration: Contact result: " . print_r($contact_result, true));
        } else {
            error_log("HubspotIntegration: Contact creation skipped - enabled: " . ($settings['enabled'] ?? 'not set') . ", object_type: " . ($settings['object_type'] ?? 'not set'));
        }
    ...
    private function createOrUpdateContact(array $data, array $settings, string $access_token, string $portal_id, int $submission_id): array {
        ...
        error_log("HubspotIntegration: Contact search response - Status: {$search_status}, Body: " . print_r($search_result, true));
        
        $contact_id = null;
        if ($search_status === 200 && !empty($search_result['results'])) {
            $contact_id = $search_result['results'][0]['id'];
            error_log("HubspotIntegration: Found existing contact ID: {$contact_id}");
        }
        ...
        error_log("HubspotIntegration: Using {$method} method for contact " . ($contact_id ? "update" : "creation"));
    ...
    public function enqueue_comprehensive_assets($hook): void {
        ...
        if (!$is_form_builder && strpos($hook, 'mavlers-cf') === false) {
            error_log("HubSpot: Not on form builder page, skipping asset enqueue");
            return;
        }
